...
void _layer_dropout_feedforward(ts_layer* l, ts_tensor* in_out, ts_layers_cache* cache) {
    if (l->training_mode && cache != NULL) {
        ...
        ts_tensor* dropout_ts_tensor = ts_tensor_create(cache->arena, s);

        ts_u64 size = (ts_u64)s.width * s.height * s.depth;
        for (ts_u64 i = 0; i < size; i++) {
            dropout_ts_tensor->data[i] = ts_prng_rand_f32() > keep_rate ? 0.0f : 1.0f;
        }

        // Applying ts_tensor to input
        ts_tensor_component_mul_ip(in_out, in_out, dropout_ts_tensor);
        ts_tensor_scale_ip(in_out, in_out, 1.0f / keep_rate);

        // Saving prev_input and dropout_ts_tensor in cache
        //ts_tensor* input = ts_tensor_copy(cache->arena, in_out, false);
        //layers_cache_push(cache, input);
        ts_layers_cache_push(cache, dropout_ts_tensor);
    }
}
void _layer_dropout_backprop(ts_layer* l, ts_tensor* delta, ts_layers_cache* cache) {
    ts_f32 keep_rate = l->dropout_backend.keep_rate;

    ts_tensor* dropout_ts_tensor = ts_layers_cache_pop(cache);

    ts_tensor_component_mul_ip(delta, delta, dropout_ts_tensor);
